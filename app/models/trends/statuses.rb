...
class Trends::Statuses < Trends::Base
  ...
  def refresh(at_time = Time.now.utc)
    ...
    Status.where('statuses.created_at >= ?', last_week).order(created_at: :desc).includes(:status_stat, :account).joins(:status_stat).where('status_stats.reblogs_count > ? OR status_stats.favourites_count > ?', 3, 3).reorder(nil).find_in_batches(batch_size: BATCH_SIZE) do |statuses|
