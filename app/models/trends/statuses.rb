...
class Trends::Statuses < Trends::Base
  ...
  def refresh(at_time = Time.now.utc)
    ...
    statuses = Status.where('statuses.created_at >= ?', 1.week.ago).order(created_at: :desc).limit(10000).includes(:status_stat, :account).joins(:status_stat).where('status_stats.reblogs_count > ? OR status_stats.favourites_count > ?', 3, 3)
