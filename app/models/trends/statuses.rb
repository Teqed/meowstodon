...
class Trends::Statuses < Trends::Base
  ...
  def refresh(at_time = Time.now.utc)
    ...
    statuses = Status.where('statuses.created_at >= ?', 8.hours.ago).order(created_at: :desc).includes(:status_stat, :account).joins(:status_stat).where('status_stats.reblogs_count > ? OR status_stats.favourites_count > ?', 3, 3)
  ...
  def calculate_scores(statuses, at_time)
    items = statuses.map do |status|
      ...
      decaying_score = if score.zero? || !eligible?(status)
                         ...
                         score * (0.3**((at_time.to_f - status.created_at.to_f) / options[:score_halflife].to_f))
                       end
